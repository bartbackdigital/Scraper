...
filename = f'{get_cwd}/funda-{current_date}.csv'
...
def proxy_request(url, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"{response.status_code}: Attempt {attempt + 1}", flush=True)
...
def read_stop_url():
    with open('start_stop.txt', 'r') as file:
        stop_url = file.read().strip()  # Read the stop URL from file and strip any whitespace
    return stop_url
...
stop_url = read_stop_url()

def urlScraper(main_url):
    if main_url == stop_url:
        print(f"Reached the stop URL: {main_url}")
        sys.exit(0)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        future_to_url = {executor.submit(listingScraper, listing_url): listing_url for listing_url in listing_urls if listing_url not in scraped_data}
    
    total_pages = resp.xpath('//ul[@class="pagination"]/li[last()-1]/a/text()').get()
    print(f'total pages : {total_pages}')
    if total_pages:
        for x in range(2, int(total_pages)+1):
            next_page_url = f'{url}&search_result={x}'
            print(next_page_url)
            req, resp = proxy_request(next_page_url)
            if req is None or resp is None:
                print(f"Failed to fetch page: {x}")
                continue
...
def listingScraper(url):
    if url == stop_url:
        print(f"Reached the stop URL: {url}")
        sys.exit(0)
...
if __name__ == '__main__':
    ...
    if os.path.exists(filename):
        scraped_data = pd.read_csv(filename)['url'].to_list()
    else:
        scraped_data = []
