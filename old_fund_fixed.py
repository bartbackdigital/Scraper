...
def listingScraper(url):
    ...
    if raw_js:
        ...
        description_texts = resp.xpath('//div[@class="object-description-body"]/text()').getall()
...
def save_image(img_url, img_id, streetAddress, addressLocality, is_first_image):
    ...
    if response.status_code == 200:
        # Generate new image name based on the address and location
        newImageName = generateImageNames(streetAddress, addressLocality)
        
        # Define the base path for saving images and ensure the directory exists
        base_image_path = f'{get_cwd}/funda_images_processed/{addressLocality}/{streetAddress}'
        os.makedirs(base_image_path, exist_ok=True)

        # Define the full path for the renamed image
        renamed_image_path = os.path.join(base_image_path, f'{newImageName}.jpg')

        # Save the downloaded image under the new name
        with open(renamed_image_path, 'wb') as f:
            for chunk in response.iter_content(1024):
                f.write(chunk)
        
        print(f"Image saved at {renamed_image_path}")

        # If it's the first image, also generate and save a thumbnail
        if is_first_image:
            try:
                img = Image.open(renamed_image_path)
                img.thumbnail((500, 500), Image.ANTIALIAS)
                thumbnail_image_path = os.path.join(base_image_path, f'{newImageName}_thumbnail.jpg')
